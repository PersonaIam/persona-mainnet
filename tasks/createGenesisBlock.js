...
fs.writeFile("config."+config.network+".autoforging.json", JSON.stringify(config, null, 2), () => {});
...
seed_peers.forEach(function(peer,index){
  ...
  fs.writeFile("config."+config.network+"."+peer.ip+".json", JSON.stringify(config, null, 2), () => {});
});


console.log('before writing ' + private_dir);

fs.writeFile("delegatesPassphrases."+config.network+".json", JSON.stringify(delegates, null, 2), () => {});
fs.writeFile("genesisPassphrase."+config.network+".json", JSON.stringify(genesis, null, 2), () => {});
